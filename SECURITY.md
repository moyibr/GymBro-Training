...
<https://github.com/moyibr/GymBro-Training/security/advisories/new>
...
- **The published images** `ghcr.io/moyibr/gymbro-api` and `-web`.
...
Read this before hosting GymBro for anyone other than yourself.
...
- **Nothing in `./data` is encrypted.** It holds `db.json` (users, passkey public keys, push
  ...
  `secret` can mint a valid session cookie for any account. **If you host GymBro for other
